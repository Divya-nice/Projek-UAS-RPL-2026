...
@php
    $layananList = [
        ['nama' => 'One Day Service',        'kategori' => 'Cuci Sepatu selesai dalam 1 hari',                   'harga' => 'Rp80.000', 'durasi' => '1 Hari',     'status' => 'Aktif',    'prefix' => 'one-day-service'],
        ['nama' => 'Deep Cleaning',           'kategori' => 'Pembersihan menyeluruh hingga ke pori pori',         'harga' => 'Rp65.000', 'durasi' => '2 - 3 Hari', 'status' => 'Aktif',    'prefix' => 'deep-cleaning'],
        ['nama' => 'Unyellowing / Whitening', 'kategori' => 'Menghilangkan noda kuning dan membuat putih kembali', 'harga' => 'Rp70.000', 'durasi' => '2 - 3 Hari', 'status' => 'Aktif',    'prefix' => 'unyellowing'],
        ['nama' => 'Women Shoes / Sandal',    'kategori' => 'Cuci sandal dan sepatu wanita',                      'harga' => 'Rp55.000', 'durasi' => '2 Hari',     'status' => 'Aktif',    'prefix' => 'women-shoes'],
        ['nama' => 'Kids Shoes / Sandal',     'kategori' => 'Cuci sepatu dan sandal anak-anak',                   'harga' => 'Rp45.000', 'durasi' => '1 - 2 Hari', 'status' => 'Aktif',    'prefix' => 'kids-shoes'],
        ['nama' => 'Leather Care',            'kategori' => 'Perawatan khusus sepatu kulit',                      'harga' => 'Rp90.000', 'durasi' => '2 - 3 Hari', 'status' => 'Aktif',    'prefix' => 'leather-care'],
        ['nama' => 'Cap Cleaning',            'kategori' => 'Cuci topi atau cap semua jenis bahan',               'harga' => 'Rp35.000', 'durasi' => '1 Hari',     'status' => 'Aktif',    'prefix' => 'cap-cleaning'],
        ['nama' => 'Sole Repaint',            'kategori' => 'Perbaikan sol sepatu (lem, jahit, press)',           'harga' => 'Rp75.000', 'durasi' => '3 Hari',     'status' => 'Nonaktif', 'prefix' => 'sole-rep'],
    ];
@endphp
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[860px] text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    @php
                        $foto = null;
                        foreach (['images/layanan', 'images'] as $folder) {
                            $found = glob(public_path($folder . '/' . $item['prefix'] . '*.{jpg,jpeg,png,webp,JPG,JPEG,PNG,WEBP}'), GLOB_BRACE);
                            if (!empty($found)) { $foto = asset($folder . '/' . basename($found[0])); break; }
                        }
                    @endphp
                    <tr class="hover:bg-slate-50/70">
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                <div class="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-slate-100">
                                    @if ($foto)
                                        <img src="{{ $foto }}" alt="{{ $item['nama'] }}" class="h-full w-full object-cover" />
                                    @else
                        ...
                        <td class="px-5 py-4 font-semibold text-slate-700">{{ $item['harga'] }}</td>
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.layanan.edit') }}" title="Edit layanan" class="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:bg-slate-50">
                                ...
                                <form action="{{ route('admin.layanan.destroy') }}" method="POST" onsubmit="return confirm('Hapus layanan {{ $item['nama'] }}?')">
                                    @csrf
                                    <button type="submit" title="Hapus layanan" class="flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 text-red-500 transition hover:bg-red-50">
