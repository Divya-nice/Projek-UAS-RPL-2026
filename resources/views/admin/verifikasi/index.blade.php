    ...
    <form method="GET" action="{{ route('admin.verifikasi') }}">
        <div class="relative">
            ...
            <input
                ...
                class="w-full rounded-lg border border-slate-200 py-2 pl-9 pr-20 text-sm text-slate-600 placeholder-slate-400 focus:border-[#1E7BC8] focus:outline-none focus:ring-2 focus:ring-[#1E7BC8]/20 sm:w-80">

            <button
                type="submit"
                class="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-[#1566AD] font-semibold">
                Cari
            </button>
        </div>
    </form>
...
@if($metode == 'transfer' && $statusPembayaran == 'menunggu_upload')

<span class="inline-flex shrink-0 rounded-full bg-slate-100 px-3 py-1 text-[11px] font-semibold text-slate-700">
    Belum Upload Bukti
</span>

@elseif($statusPembayaran == 'menunggu_verifikasi')

<span class="inline-flex shrink-0 rounded-full bg-amber-100 px-3 py-1 text-[11px] font-semibold text-amber-700">
    Menunggu Verifikasi
</span>

@elseif($statusPembayaran == 'diterima')

<span class="inline-flex shrink-0 rounded-full bg-green-100 px-3 py-1 text-[11px] font-semibold text-green-700">
    Pembayaran Diterima
</span>
...
{{-- Riwayat Pembayaran Ditolak --}}
@if($ditolakList->count())

<div class="mt-8 overflow-hidden rounded-2xl border border-red-200 bg-white shadow-sm">
    ...
    @if($ditolakList->hasPages())
        <div class="flex items-center justify-between border-t border-slate-100 px-5 py-4">
            <p class="text-sm text-slate-500">
                Menampilkan
                {{ $ditolakList->firstItem() }}
                -
                {{ $ditolakList->lastItem() }}
                dari
                {{ $ditolakList->total() }}
                pembayaran ditolak
            </p>

            {{ $ditolakList->onEachSide(1)->links() }}
        </div>
    @endif
