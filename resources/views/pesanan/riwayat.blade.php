...
<section class="min-h-[70vh] bg-[#F2F7FD] py-12 lg:py-16">
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div class="animate-[fadeInUp_0.5s_ease-out]">
            <h1 class="text-2xl font-bold text-[#1E293B] sm:text-3xl">Riwayat Pesanan</h1>
            <p class="mt-1 text-sm text-slate-500">Lihat status dan riwayat pesanan layanan cuci sepatu Anda.</p>
        </div>
                        ...
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            <div>
                                <div class="flex items-center gap-2">
                                    <span class="font-semibold text-[#1566AD]">{{ $item['kode'] }}</span>
                                    <span class="rounded-full px-2.5 py-0.5 text-[11px] font-semibold ring-1 {{ $badge }}">{{ $item['status'] }}</span>
                                </div>
                                <p class="mt-1 text-xs text-slate-400">{{ $item['tanggal'] }}</p>
                            </div>
                            <span class="text-lg font-bold text-[#1E293B]">{{ \App\Http\Controllers\PesananController::rupiah($item['total']) }}</span>
                        </div>

                        <div class="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-3">
                            <div>
                                <p class="text-xs text-slate-400">Jenis Layanan</p>
                                <p class="mt-0.5 text-sm font-medium text-[#1E293B]">{{ $item['layanan'] }}</p>
                            </div>
                            <div>
