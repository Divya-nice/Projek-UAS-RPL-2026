    ...
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-8 grid gap-6 lg:grid-cols-3">
            ...
            <div class="flex flex-col items-center rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-slate-100">
                <span class="flex h-28 w-28 items-center justify-center rounded-full bg-[#EAF3FC] text-[#1E7BC8] ring-4 ring-white">
                    <x-icon name="user" class="h-14 w-14" />
                </span>
                <h2 class="mt-4 text-lg font-bold text-[#1E293B]">{{ $user['nama'] }}</h2>
                <a href="mailto:{{ $user['email'] }}" class="mt-1 text-sm text-[#1E7BC8] hover:underline">{{ $user['email'] }}</a>
                <button type="button" class="mt-6 w-full rounded-lg bg-[#0F2A4A] px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-95">Ubah Foto Profil</button>
            </div>
