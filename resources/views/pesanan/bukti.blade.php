...
<section class="bg-[#F2F7FD] py-10 lg:py-14">
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <nav class="mb-4 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-400">
            ...
            <span>&rsaquo;</span>
            <a href="{{ route('pesanan.bayar', ['kode' => $kodeUrl]) }}" class="hover:text-[#1E7BC8]">Pembayaran</a>
            <span>&rsaquo;</span>
        ...
        @if($errors->any())
            <div class="mt-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-600">
                <p class="font-semibold">Mohon periksa kembali unggahan Anda:</p>
        ...
        <form method="POST" action="{{ route('pesanan.bukti.proses', ['kode' => $kodeUrl]) }}" enctype="multipart/form-data" class="mt-8 grid gap-6 lg:grid-cols-3">
            @csrf
            ...
            <div class="lg:col-span-2">
                <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
                    ...
                    <label for="bukti" id="dropzone"
                           class="mt-5 flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/60 px-6 py-12 text-center transition hover:border-[#1E7BC8] hover:bg-[#F2F7FD]">
            ...
            <div class="space-y-5">
                <div class="rounded-2xl bg-[#1E7BC8] p-5 text-sm text-white shadow-sm">
                    <div class="flex gap-3">
                        ...
                        <p class="leading-relaxed text-white/95">Setelah bukti pembayaran dikirim, admin akan melakukan verifikasi. Status pesanan akan berubah menjadi <strong>Menunggu Verifikasi</strong>.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-2 flex items-center justify-between gap-4 lg:col-span-3">
                <a href="{{ route('pesanan.bayar', ['kode' => $kodeUrl]) }}" class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-6 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">
